...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[Extract Expressions Request]', { aiProvider, openaiModel, hasKey: !!openaiKey })
    ...
    const systemPrompt = `You are an expert English language teacher specializing in CEFR B1-B2 level instruction. 
Extract 5-8 useful, practical English expressions from the provided transcript that would be valuable for intermediate English learners.
Focus on:
...
2. A clear, simple explanation (for B1-B2 learner)
...
Example format:
[{"phrase":"break the ice","meaning":"to start a conversation or make people feel comfortable in a new situation","example":"At the party, I told a joke to break the ice."}]`

    const userPrompt = `Here is the podcast transcript:\n\n${transcript}\n\nExtract useful expressions for B1-B2 level learners.`
